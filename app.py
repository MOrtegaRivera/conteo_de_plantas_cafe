...
if opcion_grafico == "Conteo manual vs detección automática":
    ...
    fig = px.bar(
        ...
        barmode="group",
        color_discrete_map={
            "Conteo manual en campo": "#2E86AB",
            "Detectadas por el modelo": "#A23B72"
        },
    ...
    fig.update_layout(
        legend_title_text="Tipo de sombra",
...
st.markdown(
    """
    A partir de los resultados obtenidos en la aplicación, se observa que el
    modelo YOLOv11 presenta un desempeño general favorable para el conteo
    automatizado de plantas de café. En el conjunto analizado se registraron
    **33 600 plantas contabilizadas en campo** y **32 475 plantas detectadas
    automáticamente**, lo que representa una **tasa general de detección de
    96.7 %**. La diferencia total fue de **1 125 plantas**, evidenciando una
    ligera subestimación del conteo real, aunque con una correspondencia alta
    entre el conteo manual y el resultado automatizado.

    Al analizar los resultados por finca, se identifican diferencias claras
    en el desempeño del modelo. Las mejores tasas de detección se presentaron
    en **Finca 5**, con **98.1 %**, y **Finca 2**, con **97.9 %**. Estas fincas
    también registraron los menores errores promedio, con un **MAE de 57.5**
    y **65.0 plantas**, respectivamente. Esto indica que en estos sitios el
    modelo logró una mayor cercanía con el conteo de referencia realizado en
    campo.

    En contraste, las fincas con mayor diferencia entre plantas contabilizadas
    y plantas detectadas fueron **Finca 3**, con **240 plantas de diferencia**,
    **Finca 4**, con **230 plantas**, y **Finca 6**, con **225 plantas**. En
    estos casos, las tasas de detección fueron menores, especialmente en
    **Finca 3**, que alcanzó aproximadamente **94.9 %**. Estos resultados
    sugieren que existen condiciones específicas de algunos lotes que afectan
    la capacidad del modelo para detectar correctamente todas las plantas.

    El análisis por tipo de sombra muestra que esta variable tiene una relación
    importante con el desempeño del conteo automatizado. Los lotes con
    **sombra baja** presentaron los mejores resultados, con una **tasa de
    detección de 98.8 %**, un **F1-score promedio de 0.923** y un **MAE promedio
    de 43.3 plantas**. Los lotes con **sombra media** mantuvieron un desempeño
    aceptable, con una **tasa de detección de 97.3 %**, un **F1-score promedio
    de 0.884** y un **MAE promedio de 76.0 plantas**. En cambio, los lotes con
    **sombra alta** presentaron el desempeño más bajo, con una **tasa de
    detección de 93.0 %**, un **F1-score promedio de 0.823** y un **MAE promedio
    de 153.8 plantas**.

    La pendiente del terreno también parece influir en el error del modelo.
    Los mejores resultados se concentran en lotes con pendientes bajas o
    moderadas, mientras que los mayores errores aparecen en lotes con
    pendientes más fuertes. El caso más crítico corresponde a **Finca 4 -
    Lote 2**, ubicado en la región de **Los Santos**, con **35 % de pendiente**,
    **sombra alta**, **92.0 % de detección**, **F1-score de 0.81** y **MAE de
    180 plantas**. También destacan **Finca 3 - Lote 1**, con **MAE de 170
    plantas**, y **Finca 6 - Lote 2**, con **MAE de 150 plantas**. Estos casos
    muestran que la combinación de sombra alta y pendientes fuertes puede
    reducir la precisión del conteo automatizado.

    Desde el punto de vista regional, **Valle Occidental** presentó el mejor
    desempeño general, con una **tasa de detección de 97.4 %** y un **MAE
    promedio de 75.0 plantas**. **Valle Central** y **Los Santos** tuvieron
    tasas de detección muy similares, cercanas a **96.3 %**, aunque con
    errores promedio mayores. Esto indica que la región por sí sola no explica
    completamente el desempeño del modelo, sino que deben considerarse las
    condiciones particulares de cada lote, como sombra, pendiente y estructura
    del cultivo.

    En conclusión, el conteo automatizado de plantas de café con YOLOv11 muestra
    un alto potencial como herramienta de apoyo para el monitoreo agrícola, ya
    que logra una tasa general de detección superior al **96 %**. Sin embargo,
    los resultados también evidencian que el desempeño no es uniforme en todos
    los lotes. Las principales limitaciones se presentan en sitios con
    **sombra alta** y **pendientes fuertes**, por lo que futuras mejoras del
    modelo deberían enfocarse en incorporar más muestras de entrenamiento bajo
    estas condiciones y fortalecer la validación en campo en los lotes con
    mayores errores.
    """
)